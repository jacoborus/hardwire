...
var getBlockFolders = function (dir, validator) {
 ...
 */
var getBlockPaths = function (dir, opts) {
	opts = opts || {};
	var moduleBlocks = [],
		appBlocks = [],
		validator = false;
	...
	if (opts.validator) {
		if (typeof opts.validator === 'function') {
			...
			validator = getValidator( opts.validator );
		}
	}

	if (validator) {
		moduleBlocks = getBlockFolders( dir + '/node_modules', validator );
	}
	if (opts.strict) {
		appBlocks = getBlockFolders( dir + '/app/blocks', validator );
	} else {
